...
> **→ Aktuell (18.09.): Die KI-Bewertung der Suche LÄUFT in prod — Live-Beweis: 40/41 Jobs mit Score (K2.6/Thinking-off via `noThinkingFetch`). Vollständiger Untersuchungsstand in [HANDOFF-KI-SUCHE.md](HANDOFF-KI-SUCHE.md) (§9). HR-Interview ist gehärtet und als Zwei-Phasen-Flow umgesetzt (s. Update II unten). Offen: Vercel-Env `NEBIUS_API_KEY` (tote 401-Instanz — Settings-Key ist die Quelle der Wahrheit).**
